...
optimized_base = DecisionTreeRegressor(
    max_depth=int(best_params['max_depth'])
)
optimized_adb = AdaBoostRegressor(
    base_estimator=optimized_base,
    n_estimators=int(best_params['n_estimators']),
    learning_rate=float(best_params['learning_rate']),
    random_state=42
)
optimized_adb.fit(X_train, y_train)
...
y_train_pred = evaluate_model(optimized_adb, X_train, y_train, "Train")
y_test_pred = evaluate_model(optimized_adb, X_test, y_test, "Test")
...
######################################################################################################
# Step 2: Preprocess second dataset and retrain model
...
y_perf_pred = optimized_adb.predict(X_perf_processed)
scaler_pred = MinMaxScaler()
y_perf_pred_scaled = scaler_pred.fit_transform(y_perf_pred.reshape(-1, 1))

# Append the scaled prediction to input features
X_perf_with_pred = np.hstack((X_perf_processed, y_perf_pred_scaled))

# Train a new model with augmented input
...
optimized_base2 = DecisionTreeRegressor(
    max_depth=int(best_params2['max_depth'])
)
optimized_adb2 = AdaBoostRegressor(
    base_estimator=optimized_base2,
    n_estimators=int(best_params2['n_estimators']),
    learning_rate=float(best_params2['learning_rate']),
    random_state=42
)
optimized_adb2.fit(X_train2, y_train2)

# Evaluate and visualize results for the new model
y_train2_pred = evaluate_model(optimized_adb2, X_train2, y_train2, "Train (New)")
y_test2_pred = evaluate_model(optimized_adb2, X_test2, y_test2, "Test (New)")
