...
optimized_svr = SVR(
    kernel=['linear', 'rbf', 'poly'][best_params['kernel']],
    C=best_params['C'],
...
y_train_pred = evaluate_model(optimized_svr, X_train, y_train, "Train")
y_test_pred = evaluate_model(optimized_svr, X_test, y_test, "Test")
...
y_perf_pred = optimized_svr.predict(X_perf_processed)
...
optimized_svr2 = SVR(
    kernel=['linear', 'rbf', 'poly'][best_params2['kernel']],
    C=best_params2['C'],
    epsilon=best_params2['epsilon']
)
optimized_svr2.fit(X_train2, y_train2)

# Evaluate and visualize final model
y_train2_pred = evaluate_model(optimized_svr2, X_train2, y_train2, "Train (New)")
y_test2_pred = evaluate_model(optimized_svr2, X_test2, y_test2, "Test (New)")
