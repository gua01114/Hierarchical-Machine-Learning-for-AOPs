...
optimized_gbdt = GradientBoostingRegressor(
...
optimized_gbdt.fit(X_train, y_train)
...
y_train_pred = evaluate_model(optimized_gbdt, X_train, y_train, "Train")
y_test_pred = evaluate_model(optimized_gbdt, X_test, y_test, "Test")
...
y_perf_pred = optimized_gbdt.predict(X_perf_processed)
...
optimized_gbdt2 = GradientBoostingRegressor(
    ...
    min_samples_split=int(best_params2['min_samples_split']),
    min_samples_leaf=int(best_params2['min_samples_leaf']),
    random_state=42
)
optimized_gbdt2.fit(X_train2, y_train2)

# Evaluate and visualize final model
y_train2_pred = evaluate_model(optimized_gbdt2, X_train2, y_train2, "Train (New)")
y_test2_pred = evaluate_model(optimized_gbdt2, X_test2, y_test2, "Test (New)")
