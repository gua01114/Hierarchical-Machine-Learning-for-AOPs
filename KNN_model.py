...
param_space = {
    ...
    'p': hp.quniform('p', 1, 2, 1)  # p=1 for Manhattan, p=2 for Euclidean
}
...
optimized_knn = KNeighborsRegressor(
    ...
    weights=['uniform', 'distance'][best_params['weights']],
    p=int(best_params['p'])
)
...
y_train_pred = evaluate_model(optimized_knn, X_train, y_train, "Train")
y_test_pred = evaluate_model(optimized_knn, X_test, y_test, "Test")
...
y_perf_pred = optimized_knn.predict(X_perf_processed)
...
optimized_knn2 = KNeighborsRegressor(
    n_neighbors=int(best_params2['n_neighbors']),
    weights=['uniform', 'distance'][best_params2['weights']],
    p=int(best_params2['p'])
)
optimized_knn2.fit(X_train2, y_train2)

# Evaluate and visualize final model
y_train2_pred = evaluate_model(optimized_knn2, X_train2, y_train2, "Train (New)")
y_test2_pred = evaluate_model(optimized_knn2, X_test2, y_test2, "Test (New)")
